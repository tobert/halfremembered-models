"""
Orpheus Loops Model API - Generate multi-instrumental drum loops.
...
class OrpheusLoopsAPI(ModelAPI, ls.LitAPI):
    """
    Orpheus loops model API for generating drum/percussion loops.

    Generates multi-instrumental loops, optionally with seed MIDI.
    """

    def __init__(self):
        ModelAPI.__init__(self, service_name="orpheus-loops", service_version="1.0.0")
        ls.LitAPI.__init__(self)

    def setup(self, device: str):
        """Load Orpheus loops model."""
        super().setup(device)

        logger.info("Loading Orpheus loops model...")
        models_dir = self.get_model_dir()
        self.model = load_single_model("loops", models_dir, torch.device(device))
    ...
    def predict(self, x: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self.acquire_or_busy():
            ...
            if self.tracer:
                ...
                with self.tracer.start_as_current_span("orpheus_loops.predict", context=parent_context) as span:
                    self.attach_tracking_to_span(span, client_job_id)
                    span.set_attribute("task", "loops")
    ...
    def _generate_tokens(
        ...
        # Use start token if no seed provided
        if not seed_tokens:
