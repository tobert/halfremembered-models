"""
Orpheus Mono Melodies Model API - Generate single-voice melodies.
...
class OrpheusMonoAPI(ModelAPI, ls.LitAPI):
    """
    Orpheus mono melodies model API.

    Generates single-voice melodic lines.
    """

    def __init__(self):
        ModelAPI.__init__(self, service_name="orpheus-mono", service_version="1.0.0")
        ls.LitAPI.__init__(self)

    def setup(self, device: str):
        """Load Orpheus mono melodies model."""
        super().setup(device)

        logger.info("Loading Orpheus mono melodies model...")
        models_dir = self.get_model_dir()
        self.model = load_single_model("mono_melodies", models_dir, torch.device(device))
    ...
    def predict(self, x: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self.acquire_or_busy():
            ...
            if self.tracer:
                ...
                with self.tracer.start_as_current_span("orpheus_mono.predict", context=parent_context) as span:
