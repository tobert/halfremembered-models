"""
Orpheus Bridge Model API - Generate musical bridges between sections.
...
class OrpheusBridgeAPI(ModelAPI, ls.LitAPI):
    """
    Orpheus bridge model API for generating musical transitions.

    Generates bridges that connect two musical sections.
    """

    def __init__(self):
        ModelAPI.__init__(self, service_name="orpheus-bridge", service_version="1.0.0")
        ls.LitAPI.__init__(self)

    def setup(self, device: str):
        """Load Orpheus bridge model."""
        super().setup(device)

        logger.info("Loading Orpheus bridge model...")
        models_dir = self.get_model_dir()
        self.model = load_single_model("bridge", models_dir, torch.device(device))
        self.tokenizer = OrpheusTokenizer()
        logger.info("Orpheus bridge model ready")

    def decode_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        """Parse and validate incoming request. Extract trace context before multiprocessing."""
    ...
    def predict(self, x: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self.acquire_or_busy():
            ...
            if self.tracer:
                ...
                with self.tracer.start_as_current_span("orpheus_bridge.predict", context=parent_context) as span:
                    self.attach_tracking_to_span(span, client_job_id)
                    span.set_attribute("task", "bridge")
